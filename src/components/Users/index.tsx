...
function Logos(props: { partners: Partner[] }) {
  ...
  const mobileChunks: Partner[][] = props.partners.reduce((acc, item, index) => {
  ...
  return (
    <div className={clsx("container")} style={{ paddingTop: 32, paddingBottom: 32 }}>
      <Carousel nextLabel="" prevLabel="" className={styles.swipeCardDesktop}>
      ...
      <Carousel nextLabel="" prevLabel="" className={styles.swipeCardMobile}>
        {mobileChunks.map((chunk, index) => <Carousel.Item key={index}>
          <div className="row" style={{ padding: 120, paddingTop: 80, paddingBottom: 80 }} >
            {chunk.map((partner) => <div key={partner.name} className="col col--4">
              <div className="text--center" style={{ marginBottom: 32, backgroundColor: 'white' }}>
                <img className="partner__logo" src={useBaseUrl(partner.logoPath)} alt={partner.name} />
